...
function WorkspaceItemList({ workspaceList }) {
    ...
    return (
        <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-6 cursor-pointer '>
            {
                workspaceList.map((workspace, index) => (
                    <div key={index} className='border shadow-xl rounded-xl hover:scale-105 transition-all'
                        onClick={() => OnClickWorkspaceItem(workspace.id)}
                    >
                        <Image src={workspace?.coverImage} width={400} height={400} alt='cover'
